...
object WindowTest {
  def main(args: Array[String]): Unit = {
    ...
    val resultStream = dataStream.assignTimestampsAndWatermarks(WatermarkStrategy
      ...
      .sideOutputLateData(new OutputTag[(String, Double, Long)]("later"))
          .reduce((result, data) => (data._1, result._2.min(data._2), data._3)) // 统计10秒内的最低温度值



      //    dataStream.assignTimestampsAndWatermarks(WatermarkStrategy.<SensorReading>forBoundedOutOfOrderness(Duration.ofSeconds(10))
      //      .withTimestampAssigner(new SerializableTimestampAssigner[(Long, SensorReading)] {
      //        override def extractTimestamp(element: (Long, SensorReading), recordTimestamp: Long): Long = element._2.timestamp})
      //    )
...
//      .keyBy(_._1)
    ...
    resultStream.getSideOutput(new OutputTag[String]("tag")).print("later")
